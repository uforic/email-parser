...
const credentials: Credentials = JSON.parse(readFileSync('credentials.json').toString());
    ...
    oauth2Client
        ...
        .then((a: { tokens: any }) => {
            ...
            listMessages(gmail, 'matt.sprague@gmail.com', '', async (messages) => {
                res.send(
                    'Code is ' + code + ', scope is' + req.param('scope') + ', messages are' + JSON.stringify(messages),
                );
            });
        })
        .catch((err) => {
 ...
 */
function listMessages(client: any, userId: string, query: string, callback: (a: any) => void) {
